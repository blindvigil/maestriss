...
- Sixteen built-in council presets on the party model (ordered slots, each assigning a role and a provider; duplicate roles and duplicate providers are valid; provider affinity is a soft hint only): Concord, Idea Forge, Crucible, Editorial Court, Council of X, Arcane Expedition, Scholar's Conclave, Academy, Decision Chamber, Trial by Fire, War Room, Workshop, Oracle's Table, Creative Studio, Socratic Circle, and Campaign — all producing schema-valid, deterministic Council Configurations with expert-designed v1 compositions intended for tuning after live runs. Trial by Fire and Editorial Court adopt expanded compositions; Council of X now seats X equal voting Councillors — a new seventeenth canonical role, Councillor (General Deliberator), duplicated across every seat — deliberating independently in one shared round with providers rotated in registry order, with vote casting/tallying and any non-voting synthesis stage documented as future execution work; `CouncilPreset` gains a `defaultSize` field; preset names carry no leading "The". Council suite grown to 326 assertions including exact ordered compositions for every fixed preset.
- Canonical sixteen-role council library: the approved provider-facing flavour text replaces and extends the initial six roles (Lantern Bearer, Inquisitor, Rival, Wild Mage, Royal Scribe, Magistrate — richer approved wording) and adds Empath, Saboteur (defensive failure-mode analysis), Cartographer, Archivist, Alchemist, Scout, Quartermaster, Oracle, Master of Questions, and Smith, each with practical titles, UI descriptions, default output policies, position hints (new `late` value), and soft provider affinities. The Role Grimoire enumerates all sixteen automatically with new role icons; presets are unchanged and deterministic; the council suite grew to 225 assertions including the exact role inventory, original-six id stability, and new-role composition/override fixtures.
- Council-level role flavour overrides: `CouncilConfiguration` carries an optional compact `roleFlavourOverrides` record (explicitly customized roles only; canonical text is never duplicated into configurations), validated against the role library and resolved by prompt composition ahead of canonical flavour text, so saved configurations reproduce customized role behavior anywhere without depending on browser-local state. Preset factories accept overrides as a build option, and the Role Grimoire projects local edits into the portable shape (shared converter + copy-as-JSON). Council suite grown to 171 assertions.
- Council role flavour-text library and Studio Role Grimoire editor: the provider-facing role framing moved from `RoleDefinition` into the canonical `shared/council/roleFlavourText.ts` (composed prompts byte-identical; locked snapshot unchanged), and a new Studio page lists every council role (fantasy title with the practical title always visible), edits its flavour text with a live provider-facing preview, unsaved-change indication, and revert-to-canonical, persisting edits as versioned local-browser overrides that never modify the canonical source. Runner execution uses canonical text until Council Scroll embedding; the council suite grew to 158 assertions.
