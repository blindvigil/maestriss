...
async function findComposer(page: Page): Promise<ComposerMatch> {
  ...
  throw new Error('Could not find Claude composer.');
}
...
async function verifyComposerContainsPrompt(page: Page, prompt: string) {
  ...
  const promptStart = prompt.slice(0, Math.min(prompt.length, 40));
  const verified = text.trim().length > 0 && text.includes(promptStart);
  console.log(`Text present after paste: ${text.trim().length > 0 ? 'yes' : 'no'}`);

  if (!verified) {
    throw new Error('Claude paste failed: composer did not contain prompt text');
  }

  console.log('Paste verified');
}

async function hasClaudeRespondingText(page: Page) {
  return page.evaluate<boolean>(`(() => {
    return Array.from(document.querySelectorAll('body *'))
      ...
      .some((el) => String(el.textContent || '').trim().toLowerCase() === 'claude is responding');
  })()`);
}
    ...
    const candidates = selectors
      ...
      .filter((el) => {
        ...
        return rect.width > 0 &&
          ...
          text.length > 0 &&
          !text.includes('claude is responding');
      })
      .map((el) => ({
  ...
  async waitForCompletion(page: Page) {
    try {
      ...
      while (Date.now() - startedAt < hardTimeoutMs) {
        ...
        const respondingVisible = await hasClaudeRespondingText(page);
        ...
        if (!respondingVisible && !stopVisible && sendReady && textStable) {
          return;
        }
      ...
      throw new Error('Timed out waiting for Claude completion.');
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error(`Claude waitForCompletion failed: ${message}`);
      throw error;
    }
  },
