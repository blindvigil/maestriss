...
| Commander-in-Chief (high-level project AI) | The project-level decision authority: chief architect, product strategist, technical director, project historian, design reviewer, prioritization authority, and delegation authority. Understands the whole project at a high level and briefs the implementation engineer; does not normally perform repository implementation itself. | `Reference/AI/Web_AI_Prompt.md` |
| Commander prompt | Generated operating prompt for the Commander-in-Chief: role, evidence discipline, operating style, delegation briefs, and implementation-report review. | `Reference/AI/Web_AI_Prompt.md` |
| Commander bootstrap | Generated procedural boot sequence for the Commander-in-Chief. Progressive disclosure: Tier 1 authority plus Tier 2 Commander core reach `Commander Ready`; the rest of the library loads only when a task requires it. | `Reference/AI/Web_AI_Bootstrap.md` |
...
| Council | The assembled multi-seat deliberative system — a configured Council Configuration with its Formation of seats. | `Reference/Human/02 - System Architecture.md` and AI companion |
| Seat | A stable positional identity in a Formation. A seat owns or resolves its Calling, its Preferred Mind, its provider fallback preferences, its cognitive configuration and overrides, its Memory behavior, and its maxResponseChars. | `Reference/Human/02 - System Architecture.md` and AI companion |
| Preferred Mind | A seat's configured first-choice Mind. Bound seat state; not necessarily the Execution Target that actually runs the seat, and never rewritten by fallback or by a run-level execution override. | `Reference/Human/02 - System Architecture.md` and AI companion |
...
| Transport | HOW a seat's exact composed prompt reaches a Mind and HOW the response returns: `browser` (Native Runner `/ask`) or `api` (a direct provider API, first the OpenAI Responses API). Distinct from, and never a substitute for, composition (WHAT the Mind receives). | `Reference/Human/02 - System Architecture.md` and AI companion |
| Provider family | The vendor family behind one or more Minds/models (e.g. OpenAI), which may later carry multiple models and multiple transports. Distinct from a canonical browser provider id. | `Reference/Human/02 - System Architecture.md` and AI companion |
| Model | The specific model asked over an API transport (e.g. `gpt-4o-mini`); not applicable to browser transport, which is session-defined. | `Reference/Human/02 - System Architecture.md` and AI companion |
| Execution target | The concrete target that actually executes a seat: `mindId`, `providerFamily`, optional `model`, and `transport`. Narrowly scoped execution metadata, distinct from the canonical provider/Calling/Doctrine schema; a run-level execution override forces every seat to one target without mutating configured Preferred Minds. | `Reference/Human/02 - System Architecture.md` and AI companion |
...
| `Web_AI_Prompt.md` | Generated non-authoritative prompt for the Commander-in-Chief; teaches the role and its boundary against the implementation engineer, evidence discipline, operating style and verdict vocabulary, delegation-brief format, and how to review implementation reports | Procedural local implementation workflow or project facts owned by source code and Reference documents |
| `Web_AI_Bootstrap.md` | Generated non-authoritative progressive-disclosure boot sequence for the Commander-in-Chief: Tier 1 authority, Tier 2 Commander core, a generated orientation snapshot to be verified against Tier 2, on-demand task paths, confidence model, and the Commander bootstrap report | Local editing workflow, current-state authority, or engineering truth owned by source code, `11`, the newest handoff, and the numbered Reference documents |
