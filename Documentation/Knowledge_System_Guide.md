...
| Council preset | Built-in deterministic factory producing an editable Council Configuration: ordered party slots, each assigning a role and a provider, with duplicate roles and providers valid. A user-created Council is a saved configuration under the same schema. | `Reference/Human/02 - System Architecture.md` and AI companion |
...
| Role flavour text | Provider-facing instructional text that gives a council role its perspective; canonical source is `shared/council/roleFlavourText.ts`, edited in Studio's Role Grimoire as versioned local-browser overrides and carried portably in a Council Configuration's compact `roleFlavourOverrides` record. | `Reference/Human/02 - System Architecture.md` and AI companion |
