---
Document ID: REF-16
Document Title: AI Session Bootstrap
Version: v0.3.0
Revision Date: 2026-07-10
Status: Redirect
Audience: AI
Purpose: Redirects older AI Session Bootstrap links to the two canonical generated AI onboarding artifacts.
Scope: This document no longer owns AI onboarding logic.
Related Documents:
  - AI_Prompt.md
  - AI_Bootstrap.md
  - ../Human/16 - AI Session Bootstrap.md
Current Implementation Status: Redirect only.
---
# AI Session Bootstrap

This document is retained for compatibility with older links.

The canonical AI onboarding artifacts are now:

```text
Documentation/Reference/AI/AI_Prompt.md
Documentation/Reference/AI/AI_Bootstrap.md
```

They are generated onboarding material, not independent sources of engineering truth.

Use `AI_Prompt.md` as the prompt for AI sessions in all access modes.

Use `AI_Bootstrap.md` as the procedural boot sequence for repository-aware AI work.

Do not maintain onboarding logic in this file.
