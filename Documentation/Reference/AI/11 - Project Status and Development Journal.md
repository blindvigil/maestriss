...
Audience: AI
Purpose: AI-optimized edition of the Maestriss engineering reference for Project Status and Development Journal.
...
Related Documents:
  - ../Human/11 - Project Status and Development Journal.md
...
| Field | Value |
| --- | --- |
| Canonical Document ID | REF-11 |
| Canonical Title | Project Status and Development Journal |
| Companion Human Edition | ../Human/11 - Project Status and Development Journal.md |
...
**Area:** First Direct API Execution Path (OpenAI Responses API)

**Summary:** After an isolated live smoke test proved OPENAI_API_KEY loading, authentication, billing, the official `openai` Node SDK, the Responses API, `gpt-4o-mini`, and exact `output_text` validation, added a first direct API execution transport for Council runs. A shared transport module (`runner/src/openaiTransport.ts`) owns the client, the Responses call, extraction, and structured failure classification, shared by the smoke test and Council execution. The execution seam was kept minimal: `CouncilAskFn(mindId, prompt)` is unchanged; a narrow `CouncilExecutionTarget` (`mindId`/`providerFamily`/`model`/`transport`) plus an optional `runCouncil` `executionOverride` force every Seat to one target. A run-level CLI flag `--mind openai-api` runs an all-OpenAI Council with no browser service. Composition is transport-independent: cognitive stats resolve from the configured Preferred Mind and the byte-identical composed prompt reaches the API; canonical Calling affinities, Suggested Minds, Doctrine Preferred Minds, and saved configurations are untouched. The structured result distinguishes configured Preferred Mind (never falsified), run override, and actual executing target. Forced API mode bypasses the browser fallback chain; structured OpenAI failures map into Council semantics (auth/quota/rate-limit/availability → run-unavailable circuit-break; malformed → non-availability Seat failure), only the API Mind is preflighted, and a missing key fails clearly before any Seat.

**Outcome:** Execution suite grew to 113 assertions, CLI to 69, plus a new deterministic transport suite (`npm run test:openai-transport`, 27, no network). The isolated live `npm run test:openai-api` passes end-to-end after the refactor. All council suites, baton, doctrine-memory audit, both builds, and the version verifier pass. No live Dream Lab Council run was performed. Provider/Mind/model/transport coupling was found only at the transport layer (`stage.provider` is Mind identity and was passed straight to the browser `ask`); the schema was not touched — the fix was a narrow execution-target concept distinct from Mind identity.

**Lessons Learned:** The seam was already right — `runCouncil` delegates HOW to the injected `ask`. Forcing a transport is a run-level override plus a dispatcher, not a framework. Keeping "configured Preferred Mind" (schema identity) separate from "execution target" (family/model/transport) is what lets one family carry multiple models/transports later without a schema rewrite, and keeping composition strictly before transport selection is what makes the composed prompt byte-identical across transports.

### 2026-07-14

**Area:** Run-Scoped Provider Availability Memory for Council Runs

**Summary:** Established provider unavailability as a routine operating condition rather than an edge case. Added a run-scoped availability registry (`runner/src/councilAvailability.ts`) — created fresh per run, never persisted — recording each Mind as `unknown`/`ready`/`unavailable` with structured evidence (reason, message, source `preflight`/`readiness`/`ask`, run-relative first-detected time). An opt-in preflight (`runCouncil({ preflight: true })`, enabled by `council run`) does one readiness pass over the Minds that can appear in the Formation using the existing Runner readiness detection — no Council-local provider logic and no prompt sent; the existing taxonomy already classifies the Claude/Perplexity Cloudflare loop as `security-verification` and the Copilot tenant block as `provider-blocked`. Each seat derives an effective execution chain (configured chain minus the run's unavailable Minds, order preserved); a Mind established unavailable — at preflight, a readiness gate, or a fallback-eligible ask failure — is skipped on every later seat with no re-check, no ask, and no retry consumed. Filtering sits strictly before the failure policy and reuses `providerUnavailableAskReasons` as the single circuit-break source of truth; timeouts and refusals never mark a Mind unavailable, `unknown` stays executable, and `ready → unavailable` transitions are caught at ask time. The configuration, `providerFallbacks`, and each seat's Preferred Mind are never mutated. Structured state/events (`providerAvailability`, `CouncilPreflightStart`/`ProviderAvailability`/`PreflightComplete`/`ProviderSkipped`, `effectiveChain`) feed the CLI (start-of-run CHECKING MINDS check, grouped READY/UNAVAILABLE/UNKNOWN summary, per-seat configured-vs-effective chain, concise later-seat skip lines) and a future Studio UI.

**Outcome:** Execution suite grew to 99 assertions, CLI to 63 (contract 401 and cognitive-stats 105 unchanged). All council suites, the baton suite, the doctrine-memory audit, both builds, and the version verifier pass. No live Council run was performed. Behaviour with no preflight and no learned unavailability is unchanged (legacy per-seat readiness path preserved).

**Lessons Learned:** The clean seam was to layer run memory over the existing per-seat readiness/fallback machinery rather than replacing it: preflight is opt-in, the legacy per-seat gate is untouched when preflight is off, and every existing assertion kept passing. Separating "configured chain" (never mutated) from "effective chain" (derived per seat) is what keeps saved Councils and canonical chains authoritative while availability is run-scoped.

### 2026-07-14

**Area:** Canonical Per-Seat Response-Length Target (maxResponseChars)

**Summary:** Added the approved seat-level `maxResponseChars` field: a prompt-side requested response-size ceiling in characters, defaulting to the shared `defaultMaxResponseChars` (1024) through one pure resolver, with explicit values validated as integers in 32-8192 and existing configurations remaining valid untouched. Every seat prompt now carries one deterministic instruction ("Keep your complete response within approximately N characters. Prioritize the most important content if space is limited." — raw integer, characters, never tokens) as its own layer between cognitive guidance and the output instruction. The target is explicitly not max_tokens, not native provider enforcement, and not truncation: over-target responses still pass, contributions enter Council history unchanged, and the overrun is recorded structurally (`responseChars`, `responseTargetExceeded`, `resolvedMaxResponseChars` in composition metadata) for the CLI today and Studio later. Voice independence is asserted in both directions. Compose-once fallback means every fallback Mind receives the byte-identical prompt with the same target. The CLI shows the resolved target per seat, reports actual length against it, and warns with the exact overage when exceeded.

**Outcome:** Contract suite grew to 401 assertions, cognitive-stats to 105, execution to 80, CLI to 55; locked prompt snapshots were intentionally re-locked with the response-length line. All suites, the baton suite, the doctrine-memory audit, both builds, and the version verifier pass. Doctrine seats deliberately carry no explicit values — all sixteen inherit the 1024 default pending live tuning.

**Lessons Learned:** The live-run audit showed a short-response assumption fossilized as detector geometry; keeping maxResponseChars strictly prompt-side (and overruns diagnostic) preserves the detectors' obligation to handle long responses while still shortening Council rounds by default.

### 2026-07-14

**Area:** Deterministic Provider Fallback for Council Seats

**Summary:** Discovered during live Council testing (Copilot blocked by the current account plan) that provider-level unavailability — plan restrictions, usage and rate limits, login/session failures, outages — is inevitable in normal use, and implemented ordered provider fallback. Canonical seat model: a Seat is a stable positional identity owning its Calling (cognitive duty), its preferred Mind (bound AI), its cognitive overrides, and its fallback preferences; the preferred Mind is bound seat state but not guaranteed to be the executing provider. `provider` (the preferred Mind) plus additive `providerFallbacks` (ordered alternate Minds) form one deterministic effective chain of up to five choices, validated against the canonical registry (no duplicates, no unknown ids, no silent correction). Calling `providerAffinity` became the canonical five-provider role-sensitive ranking and Doctrine factories derive every built-in seat's chain from it. The seat prompt is composed exactly once, from the preferred provider's identity (Calling flavour, cognitive stats, context, Memory, budgets all unchanged), and every chain provider receives it byte-identical; contributions are attributed to the provider that actually produced them. Fallback eligibility is strictly availability-scoped (non-ready readiness, or the explicit `providerUnavailableAskReasons` ask-failure taxonomy); availability rejections consume no retry budget, executing providers keep normal retry-once/skip-and-record/halt semantics, availability failures on an executing provider advance the chain instead of retrying, and an exhausted chain resolves via the failure policy as `provider-unavailable`. Structured records and events (`CouncilSeatProviderSelection`, `CouncilSeatBegin`, `CouncilProviderRejectedEvent`) carry preferred provider, chain, ordered rejections with reasons, executing provider, and exhaustion for the CLI today and a future Studio live UI (presentation deliberately undesigned). Per explicit product decision, Reka is the Empath's default Mind (Suggested AI and affinity ranking now Reka-first). The CLI reports chains, rejections, transitions with a seat-unchanged notice, executed-versus-preferred provider, chain exhaustion, and a verbose FNV-1a prompt-identity reuse proof.

**Outcome:** Execution suite grew to 72 assertions (fallback order, exact prompt reuse, retry-budget preservation, exhaustion under both policies, conservative non-availability behavior, single-provider compatibility), the CLI suite to 51, and the contract suite to 388 (chain validation, affinity rankings, per-Doctrine derived-chain verification). All council suites, the baton suite, both builds, and the version verifier pass. One deliberate semantic refinement: a structured availability ask failure is no longer retried by retry-once — it advances the chain (or exhausts it), because retrying an unavailable provider is what fallback replaces.

**Lessons Learned:** Composing the prompt before provider selection made prompt-identity across fallbacks a construction guarantee rather than a test hope; and an explicit availability taxonomy kept fallback from quietly becoming a try-anything retry loop.

### 2026-07-14

**Area:** Live Council CLI Observability

**Summary:** Rebuilt the `council run` operator experience for live two-terminal testing. Terminal 1 remains the Runner service's always-on low-level lifecycle view (request ids, readiness, paste/submission evidence, completion detection, extraction); Terminal 2 now renders a high-level orchestration view from structured execution records: run header with ordered Formation, per-seat blocks showing the Calling, AI, input policy with practical explanation, all six resolved cognitive stats (numeric level plus display-only descriptor from `shared/council/cognitiveStatDescriptors.ts`), and context-flow diagnostics that distinguish input-policy eligibility, Memory selection, budget inclusion, Memory exclusions, budget omissions, and truncation (original size versus applied cap). While an ask is in flight the CLI prints a 10-second elapsed heartbeat that stops the moment the ask resolves. Retry, skip, and halt decisions are stated explicitly (including that a retry re-sends the same exact prompt with no contribution recorded), every seat reports elapsed time and response length, a running pass/skip/fail summary follows each seat, and the final summary reports seat counts, elapsed time, halted-seat detail on FAIL, skipped seats on PARTIAL, the last successful contribution, and the retained Crown Council honesty note. `--verbose` adds composition diagnostics (pipeline-stage contribution ids, budget usage, per-contribution cap, resolved cognitive instruction lines) and the exact provider-facing prompt; normal mode never prints the prompt but always shows the full extracted response. The engine gained additive observability metadata only (`CouncilSeatComposition`, `elapsedMs`, `responseChars`, injectable clock, per-attempt `onSeatAttempt` callback); execution semantics are unchanged and re-asserted.

**Outcome:** New deterministic suite `npm run test:council-cli` (41 assertions) drives the real reporter through the real engine with fake asks and an injected clock; the execution suite grew to 57 assertions covering the new structured metadata. All council suites, the baton suite, both builds, and the version verifier pass. No live run has exercised the new output yet.

**Lessons Learned:** Rendering the CLI from structured seat records instead of ad hoc strings cost little and leaves the future Studio UI a ready-made data contract — the reporter is the first consumer of the run-record shape that persistence will snapshot next.

### 2026-07-14

**Area:** Canonical Six Cognitive Stats and Their Prompt Effect

**Summary:** Implemented the six canonical cognitive stats — Temperament (Precise/Imaginative), Voice (Terse/Expansive), Conviction (Adaptable/Resolute), Dissent (Cooperative/Adversarial), Depth (Swift/Exhaustive), and Memory (Isolated/Full Council) — as ten-level (0–9, 5 neutral) Maestriss behavioral dimensions in `shared/council/cognitiveStats.ts` and `cognitiveGuidance.ts`. One pure resolution function applies the canonical precedence seat override > Calling default > provider default > neutral; Suggested AI never participates. All nine providers carry approved product-default calibrations (Google deliberately neutral; documented as product defaults, not provider psychology), all sixteen Callings carry approved partial defaults, and the approved choreography-sensitive seat overrides landed on Dream Lab's Wild Mages, Trial by Fire's second Saboteur, Oracle's Table's second Oracle, Socratic Circle's second Sage, and every Crown Council Magistrate (Memory 0). Prompt composition renders a sparse "Cognitive guidance:" section from a fixed source-checked instruction catalog (neutral levels emit nothing; raw numbers are never sent), kept strictly separate from Calling flavour text, and Memory acts mechanically by narrowing input-policy-eligible prior contributions (never expanding eligibility; Memory 5 preserves input-policy behavior exactly; independent-round stays isolated at every level). Migration: Voice superseded `responseLength` and the ten-level Dissent stat superseded the three-level `dissent` variable, both removed cleanly because no Council Scroll persistence has shipped; Calling intensity (`roleIntensity`) and input mode remain separate. Composition now returns `resolvedCognitiveStats`, `eligibleContributionIds`, and `memorySelectedContributionIds` diagnostics.

**Outcome:** A new deterministic suite, `npm run test:cognitive-stats` (100 assertions), covers vocabulary, level validation, exact provider/Calling data, resolution precedence and immutability, the locked instruction catalog extremes, sparse injection, the full Memory exposure mapping with legacy-behavior regressions, the responseLength/dissent migration, Doctrine choreography overrides, and exact sparse and deviating prompt snapshots. The council contract suite was migrated intentionally (locked snapshot now carries the guidance block) and passes alongside the execution suite, baton suite, both builds, and the version verifier. Current execution expresses stats through provider-facing wording and context exposure only; native API parameter mapping remains future work for an API-backed execution layer.

**Lessons Learned:** Because provider and Calling defaults are rarely all-neutral, most real seats now carry a guidance block — sparse injection controls prompt noise per stat, not per seat; and separating eligibility (input policy) from exposure (Memory) kept the mechanical layer honest with zero changes to the execution engine.

### 2026-07-14

**Area:** Live Doctrine Execution from the Runner CLI

**Summary:** Implemented the first live Council execution vertical slice: `npm run dev -- council run --doctrine <id> (--prompt "text" | --prompt-file path) [--size N] [--verbose]`. A new execution engine (`runner/src/councilExecution.ts`) follows the baton test architecture — a pure engine over an injected ask function — and the CLI runs it over the existing `/ask` lifecycle, reusing provider drivers, extraction, readiness, cancellation, and timeout semantics without duplication. Seats execute sequentially in Formation order; each seat prompt is composed by the shared pipeline against the real prior contributions selected by its input policy, and the exact provider-facing prompt is preserved on the in-memory seat record (`--verbose` prints it before sending). Failure policies: halt stops immediately preserving real prior contributions; retry-once re-sends the same exact prompt once and halts if the retry fails (the schema does not define a post-retry alternative, so the safest reading applies); skip-and-record continues without forwarding anything from the failed seat. Deterministic composition failures (budget errors) are never retried. Runs classify as PASS / PARTIAL / FAIL, contributions are only ever actual extracted responses, and a Crown Council run explicitly reports that vote aggregation is not implemented — its last output is an individual opinion, not a collective verdict. Run state is in-memory only; persisted Council Records are the next slice.

**Outcome:** A new deterministic suite, `npm run test:council-execution` (57 assertions), covers Formation order, exact prompt transmission, all six input policies, flavour-override effect on sent prompts, Formation-provider authority over Suggested AI, all three failure policies, readiness gating, result classification, configuration immutability, determinism, and Crown Council honesty. Contract suite, baton suite, both builds, and the version verifier pass. No live Doctrine run has been executed yet; the first manual target is Dream Lab.

**Lessons Learned:** The injected-ask pattern proven by the baton test transferred directly to council execution — the entire engine is testable without a browser precisely because the ask boundary is the only side effect.

### 2026-07-14

**Area:** Canonical Vocabulary Migration (Doctrine / Calling / Formation / Seat)

**Summary:** Migrated the entire Council domain to the canonical vocabulary: presets became Doctrines, roles became Callings, and the Doctrine → Formation → Seat → Council model is now the product language everywhere. The library holds exactly sixteen Callings (Master of Questions → Sage, Scout → Pathfinder, Smith → Architect; Councillor/General Deliberator removed — equal deliberation is a Doctrine mechanic, not a Calling) and exactly sixteen Doctrines (Concord → Realm Summit, Idea Forge → Dream Lab, Editorial Court → Imperial Court, Council of X → Crown Council, Academy → Grand Academy, Creative Studio → Creation Chamber, Campaign → Grand Campaign). Crown Council now seats all-Magistrate peers in one independent round with providers rotated in registry order. Shared modules were renamed (`callings.ts`, `callingFlavourText.ts`, `doctrines.ts`), schema fields renamed (`calling`, `doctrineId`, `callingFlavourOverrides` — justified by zero persisted artifacts), and Studio's Role Grimoire became the Calling Grimoire. Deliberately retained compatibility identifiers: the `maestriss.roleFlavourOverrides` localStorage key, parser-level legacy calling-id migration, and the internal `roleIntensity` variable name. Calling-level Suggested AI metadata (single default best-fit provider, advisory only) is carried on each Calling.

**Outcome:** The council suite asserts the sixteen/sixteen inventory, superseded-name absence, Crown Council mechanics, and legacy-override migration; runner and Studio builds pass. Dream Lab seat 5 changed from Rival/Gemini to Pathfinder/Gemini during Formation review.

**Lessons Learned:** Renaming vocabulary before any persistence exists is dramatically cheaper than after; the schema-field rename window closes when the first Council Scroll is saved to disk.
