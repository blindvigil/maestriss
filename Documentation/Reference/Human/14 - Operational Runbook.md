...
Audience: Human
Purpose: Human-oriented edition of the Maestriss engineering reference for Operational Runbook.
...
**Onboarding note:** New maintainers should start with `Documentation/Reference/Human/Start_Here.md`, then read the most recent milestone handoff in `Documentation/Handoffs/` for current project state, before using this runbook for specific procedures. High-level project AI sessions should use `Documentation/Reference/AI/Web_AI_Prompt.md`, then `Documentation/Reference/AI/Web_AI_Bootstrap.md`, then `Documentation/Reference/AI/Start_Here.md`. VS Code or repository-attached engineer AI sessions should use `Documentation/Reference/AI/VSC_AI_Prompt.md`, then `Documentation/Reference/AI/VSC_AI_Bootstrap.md`, then `Documentation/Reference/AI/Start_Here.md`.
...
Exactly one of `--prompt` or `--prompt-file` is required (`--prompt-file` reads UTF-8 text); `--size` applies only to Crown Council; unknown Doctrine ids fail with the list of valid ids. The sample prompt file `runner/test-prompts/response-detection.txt` (the response-detection brainstorm brief) ships in the repository. Output is rendered from structured execution records (`runner/src/councilCliFormat.ts`), the same records a future Studio UI will consume; the actual extracted response of every seat remains fully visible in normal mode. Every built-in seat carries an ordered provider fallback chain (up to five choices, derived from the Calling's canonical provider ranking): when a provider is unavailable (readiness not ready, or a structured availability failure such as an account/plan block, login problem, usage or rate limit, or capacity error), the CLI reports the rejection and tries the next choice with the byte-identical composed prompt — the seat's Calling, cognitive stats, and context never change — and states the executing provider versus the seat's preferred Mind on completion. Every seat requests a response-size target (default 1024 characters; per-seat maxResponseChars 32-8192): the CLI shows the resolved target, reports the actual response length against it, and prints a clear warning when the target is exceeded — overruns are diagnostic only and never fail the seat or truncate the contribution. If every choice is unavailable the CLI prints PROVIDER CHAIN EXHAUSTED with each provider's structured reason and the seat's failure policy resolves the outcome. Non-availability failures (timeouts, refusals, empty extractions) never trigger fallback.

Provider unavailability is treated as routine, and availability is remembered for the whole run. At the start of `council run` the CLI runs one preflight readiness pass over every Mind that can appear in the Formation and prints a "CHECKING MINDS..." check and a grouped **Mind availability** summary (READY / UNAVAILABLE / UNKNOWN with an "Available Minds: X of N" count) — no prompt is sent during preflight. Any Mind established unavailable (the Cloudflare "verify you are human" loop shows as `security-verification`, a tenant block as `provider-blocked`, and login/plan gates likewise) is then skipped on every later seat with no re-check and no ask, so a down Mind is never re-attempted seat after seat. Each seat shows its configured chain and, when they differ, the effective chain (configured minus the run's unavailable Minds) while keeping the Preferred Mind visible; a Mind that goes down mid-run (for example a usage limit at ask time) is reported once and then shown as a concise "already unavailable for this run" skip on later seats. Unknown Minds stay eligible — the first real ask establishes their state — and the saved Council and its provider chains are never modified; the effective chain is derived at runtime.
...
This mode requires no Runner browser service. It reads `OPENAI_API_KEY` from the environment (never printed or stored; API billing is separate from any ChatGPT subscription) and fails clearly before any seat if the key is missing.

Operational note: on Windows, a key saved at **user** scope is not picked up by terminals or editor processes that were already running when it was saved. If the run reports `missing-api-key` even though the variable is set, open a fresh terminal (or otherwise reload the environment) and retry. Maestriss reads the key only from the process environment and never stores it in the repository. Prompt composition is identical to the browser path — each seat keeps its canonical Calling, cognitive stats, `maxResponseChars`, Memory/context policy, and speaking order, and the byte-identical composed prompt is what is sent; only the executing target changes. The CLI prints a RUN EXECUTION OVERRIDE banner and, per seat, the configured Preferred Mind alongside the OpenAI API execution Mind/model/transport. Only the OpenAI API Mind is preflighted (no browser providers); the normal provider fallback chain is bypassed, so if an OpenAI request fails the seat's own failure policy resolves it (structured OpenAI auth/quota/rate-limit/availability failures also mark the API Mind unavailable for the rest of the run). Canonical Calling affinities, Doctrine Preferred Minds, and saved Councils are never modified; `maxResponseChars` stays a prompt-side target and is not mapped to a native token limit.
