...
Every Council seat carries a requested response-size ceiling, `maxResponseChars`, measured in characters. The shared canonical default is `defaultMaxResponseChars` (1024); an explicit seat value must be an integer between 32 and 8192, and configurations without the field remain valid and simply inherit the default through the single pure resolver (`resolveMaxResponseChars`). This is a prompt-side target rendered as one deterministic provider-facing instruction ("Keep your complete response within approximately N characters. Prioritize the most important content if space is limited." — raw integer, characters, never tokens), placed as its own prompt layer between cognitive guidance and the output instruction. It is NOT max_tokens, NOT a native provider API parameter, and NOT a truncation mechanism: actual provider output is always preserved unchanged, an over-target response still passes the seat and enters Council history intact, and the overrun is recorded as a diagnostic (`responseChars` and `responseTargetExceeded` on the seat record; `resolvedMaxResponseChars` in composition metadata — all structured state a future Studio UI can display). The target is independent from the Voice cognitive stat: Voice governs style and degree of elaboration, `maxResponseChars` governs requested physical size (Voice 9 with a 512-character target is expansive style in a small space; Voice 1 with 8192 is terse style despite ample room). Memory and prompt budgets separately govern how much prior Council material later seats receive. Because the seat prompt is composed exactly once before provider selection, every fallback Mind receives the byte-identical prompt and therefore the exact same response-length target.

Current limitation (verified live, 2026-07-15): because the target is advisory prompt guidance and nothing bounds what enters Council memory, a provider may substantially exceed it and the oversized contribution is still admitted in full. In the first live all-OpenAI Dream Lab run, `gpt-4o-mini` repeatedly returned roughly twice the 1,024-character target; the overruns were reported correctly by the CLI but consumed the total prompt budget, so a seat configured with Memory 9 (full eligible record) did not receive the full eligible record. Observation is not enforcement: `maxResponseChars` is not currently an end-to-end Council invariant. Making it one is the current highest-priority decision — see `11 - Project Status and Development Journal.md` and the 2026-07-15 handoff. Any three-layer prompt/transport-budget/admission-boundary design discussed there is proposed only and is not implemented.
...
The canonical seat model: a Seat is a stable positional identity; its Calling is the cognitive duty bound to that seat; its Mind is the preferred AI/provider bound to that seat; and its fallback chain lists the ordered alternate Minds permitted to execute the same seat when the preferred Mind is unavailable. A seat therefore owns its Calling, its preferred Mind, its cognitive overrides (including Voice, the verbosity configuration), its policies, and its fallback preferences — and the same Calling may occupy multiple seats with different preferred Minds and different cognitive overrides. The preferred Mind is bound seat state; it is simply not guaranteed to be the executing provider. `CouncilStage.provider` is the preferred Mind and the additive `providerFallbacks` list extends it into one deterministic effective chain (`effectiveProviderChain`: preferred first, then fallbacks, at most five total choices). Validation rejects unknown provider ids, duplicates anywhere in the effective chain, and more than five choices — nothing is silently deduplicated or discarded. Each Calling's `providerAffinity` is the canonical five-provider ranking for that Calling, and the Doctrine factories derive every built-in seat's fallback chain from it (assigned provider first, then the ranking minus that provider), so fallback order stays role-sensitive without duplicating rankings across Doctrine definitions; per-seat customization lives on the stage's own `providerFallbacks`.

Fallback is strictly limited to provider-availability conditions: any non-`ready` readiness status, or an ask failure whose structured category is in the explicit `providerUnavailableAskReasons` list (plan/account blocks, login and session problems, usage and rate limits, capacity errors). Composition failures, invalid configurations, timeouts, refusals, empty extractions, and other Maestriss or execution failures are never disguised by fallback. The seat prompt is composed exactly once, before provider selection, with cognitive stats resolved from the preferred provider — so a fallback Mind executes the exact unchanged seat — same stable seat identity, same Calling, same cognitive stats (including Voice), same context selection and Memory exposure, same budgets, and the byte-identical composed prompt; only the executing provider changes, and the contribution is attributed to the provider that actually completed the seat.

Fallback resolves before the seat's failure policy: availability rejections (readiness or ask-phase) consume no retry budget and advance the chain; once a provider actually executes the seat, normal `retry-once` / `skip-and-record` / `halt` semantics govern that executing provider, and a non-availability failure never advances the chain. An exhausted chain (every configured choice unavailable) records `failureReason: provider-unavailable` and resolves via the failure policy — skipped under skip-and-record, halting under halt or retry-once, since no available provider remains to retry. The full selection state (`CouncilSeatProviderSelection`: preferred Mind, configured chain, effective chain, ordered structured rejections with reasons, executing provider, fallback-used flag, chain exhaustion) is carried on structured seat records and events (`CouncilSeatBegin`, `CouncilProviderRejectedEvent`) so a future Studio UI can render live fallback feedback without parsing CLI text; the final Studio presentation remains an undesigned future product decision.

#### Run-Scoped Provider Availability

Provider unavailability is a routine operating condition of Maestriss, not an exceptional edge case: on a given day several Minds may sit in a Cloudflare "prove you're human" loop, be logged out, be plan- or account-gated, or be blocked for the tenant. So a Council run carries a run-scoped availability memory (`runner/src/councilAvailability.ts`): a registry, created fresh for each run and never persisted, that records each Mind as `unknown`, `ready`, or `unavailable`, retaining structured evidence for unavailable Minds (reason, optional message, source `preflight`/`readiness`/`ask`, and a run-relative first-detected timestamp). A new Council run begins with a new registry; nothing carries between runs, and the saved Council Configuration, its `providerFallbacks`, and each seat's Preferred Mind are never mutated.

Availability is established two ways. When the engine is run with preflight enabled (the `council run` CLI does this), one readiness pass at Council start inspects every Mind that can appear anywhere in the Formation's configured chains — no more — using the existing Runner readiness detection rather than any Council-local provider logic. The existing readiness taxonomy already identifies the operating conditions that matter: the Claude/Perplexity Cloudflare verification loop as `security-verification`, the Copilot tenant block as `provider-blocked`, and login/session/plan gates as `logged-out`/`unsupported-browser-login`/`provider-blocked`. Preflight never sends a prompt; it only classifies `ready`, `unavailable`, or `unknown`. Second, availability is learned during execution: a per-seat readiness rejection or a fallback-eligible structured ask failure marks the Mind unavailable for the rest of the run, and a Mind that passes preflight may still transition `ready → unavailable` at ask time (a usage limit or account gate appearing mid-run). `unknown` never means unavailable — an unknown Mind stays executable, and its first real ask establishes its state. For this slice, `unavailable` is monotonic within a run: there is no automatic mid-run recovery or re-probing.

From this, each seat derives its effective execution chain at runtime — the configured chain minus the Minds already unavailable this run, preserving configured order. A Mind established unavailable is skipped on every later seat with no fresh readiness check, no ask, and no retry budget consumed: a run-scoped circuit breaker so later seats never re-attempt a Mind already known down. This filtering is layered strictly before the seat failure policy and reuses `providerUnavailableAskReasons` as the single source of truth for ask-phase circuit-breaking — it never marks a Mind unavailable for a generic timeout, a refusal, a composition error, or any other non-availability failure. When every configured choice is unavailable the chain is exhausted and the existing failure policy resolves the seat exactly as before (`provider-unavailable`). The run-scoped state and its transitions are exposed as structured records and events (`providerAvailability` on the run result; `CouncilPreflightStart`, `CouncilProviderAvailabilityEvent`, `CouncilPreflightComplete`, `CouncilProviderSkippedEvent`; `effectiveChain` on `CouncilSeatBegin` and the seat selection record) for both the CLI and a future Studio live presentation, which remains an undesigned product decision.

#### Execution Transport and the OpenAI API Path

Council composition owns WHAT a Mind receives; transport owns HOW the exact composed prompt reaches the Mind and HOW the response returns. These are separate concerns, and the composed Seat prompt is transport-independent: it is composed exactly once, before any transport is chosen, so the byte-identical prompt is eligible for either transport. There is no special API prompt — Calling framing, cognitive guidance, the `maxResponseChars` guidance, Memory/context material, and the original request are always present regardless of transport.

Maestriss now has two execution transports. The Native Runner browser path (`/ask`) drives provider web surfaces and remains the default. The first direct API path is the OpenAI Responses API (`runner/src/openaiTransport.ts`, official `openai` Node SDK, initial model `gpt-4o-mini`); the shared transport owns client construction, the Responses API call, `output_text` extraction, and structured failure classification, and is shared with the isolated live smoke test. `createOpenAiCouncilAsk` adapts it to the existing execution seam, `CouncilAskFn(mindId, prompt)` — the seam is unchanged; only the injected implementation differs.

Transport is deliberately distinct from Mind identity so one provider family can later carry multiple models and multiple transports. A `CouncilExecutionTarget` (`mindId`, `providerFamily`, optional `model`, `transport: 'browser' | 'api'`) is the concrete execution target. A browser target's `mindId` is the canonical provider id (so existing behaviour is unchanged); an API target uses a distinct execution-mind id such as `openai-gpt-4o-mini`. This is narrowly scoped execution metadata — NOT a redesign of the canonical provider/Calling/Doctrine schema, which is unchanged.

A run-level execution override (`runCouncil` `executionOverride`, surfaced by the CLI as `--mind openai-api`) forces every Seat through a single target and does NOT walk the Seat's configured browser fallback chain. Every Seat keeps its canonical Calling, cognitive configuration, `maxResponseChars`, Memory/context policy, and speaking order; Doctrine choreography and canonical Calling affinities, Suggested Minds, and Doctrine Preferred Minds are unchanged; the saved configuration is never mutated. The structured record distinguishes the configured Preferred Mind (never falsified), the run override, and the actual executing target (`executionOverride` and `executedTarget` on the seat selection record; `executionOverride` on the run result). Under a forced API override an OpenAI failure applies the Seat's existing failure policy and is never silently redirected to a browser provider — transport-aware fallback across families is a separate future design. This integrates with run-scoped availability: only the override target is preflighted (no unrelated browser providers, and no paid dummy generation — the first real Seat ask establishes dynamic availability), a missing `OPENAI_API_KEY` fails clearly before any Seat, and structured OpenAI failures map into Council semantics deliberately — authentication, billing/quota, rate limit, and network/availability become run-unavailable reasons that circuit-break the API Mind for the run, while a malformed response is a non-availability failure owned by the Seat failure policy and never disguised as provider unavailability. `maxResponseChars` remains a prompt-side behavioral target and is not mapped to `max_output_tokens`; native inference-parameter mapping is future work. `OPENAI_API_KEY` is read from the environment and never stored by Maestriss, and API billing is separate from any ChatGPT browser subscription.
