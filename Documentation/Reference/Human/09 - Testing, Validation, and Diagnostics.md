...
Audience: Human
Purpose: Human-oriented edition of the Maestriss engineering reference for Testing, Validation, and Diagnostics.
...
Related Documents:
  - ../AI/09 - Testing, Validation, and Diagnostics.md
...
The shared council orchestration contract has its own deterministic suite (`npm run test:council`, `runner/src/councilAssertions.ts`): canonical provider-registry equivalence, schema validation accept/reject cases, Doctrine and Formation invariants, Calling metadata and refusal-safety wording checks, Calling/flavour-text bijection and override-envelope semantics (immutable edits, revert, versioned persistence parsing, legacy calling-id migration), council-level `callingFlavourOverrides` validation and resolution precedence, and prompt-composition behavior including input policies, budgets, exact truncation and omission notices, precedence, and a locked full-prompt snapshot. The Council execution engine has a second deterministic suite (`npm run test:council-execution`, `runner/src/councilExecutionAssertions.ts`) that runs the real engine over injected fake asks, following the baton assertion architecture: Formation-order execution, exact composed-prompt transmission, every input policy, flavour-override effect on the sent prompt, halt/retry-once/skip-and-record semantics, readiness gating, provider preference chains and fallback (availability-only eligibility, exact prompt reuse across providers, retry-budget preservation, chain exhaustion, structured selection records), run-scoped provider availability (fresh-per-run registry; preflight classification of ready/unavailable/unknown; configured chains never mutated; effective chain removes unavailable Minds while preserving order; Preferred Mind retained even when unavailable; already-unavailable Minds skipped with no ask and no retry; a first-seat ask failure circuit-breaking a Mind for later seats; ready→unavailable transitions at ask time; timeouts and refusals never marking a Mind unavailable; all-unavailable chain exhaustion resolving through the unchanged failure policy; byte-identical prompt reuse through a run-scoped skip; structured availability events), PASS/PARTIAL/FAIL classification, response-target diagnostics (resolved maxResponseChars in composition metadata; over-target responses pass with responseTargetExceeded recorded and contributions never truncated), configuration immutability, determinism, and Crown Council vote-aggregation honesty.  A third deterministic suite (`npm run test:cognitive-stats`, `runner/src/cognitiveStatsAssertions.ts`) covers the six cognitive stats: the canonical vocabulary and ten exact levels, provider/Calling/seat data shapes, the seat > Calling > provider > neutral resolution precedence, the locked instruction catalog, sparse guidance injection, mechanical Memory exposure versus input-policy eligibility (including the Memory-5 preserves-legacy-behavior regression), the responseLength/dissent migration, the approved Doctrine choreography overrides, and exact sparse and deviating prompt snapshots. A fourth deterministic suite (`npm run test:council-cli`, `runner/src/councilCliFormatAssertions.ts`) drives the Council CLI reporter through the real engine with fake asks and an injected clock, asserting the operator output: pre-run header and Formation, seat progress blocks, cognitive-stat display, input-policy explanations, context-flow diagnostics (Memory-excluded versus budget-omitted versus truncated), retry/skip/halt decisions, provider fallback transitions and chain-exhaustion output (including the prompt-identity reuse proof in verbose mode), running and final summaries, response-target display and over-target warnings, the run-scoped availability output (start-of-run CHECKING MINDS check and grouped READY/UNAVAILABLE/UNKNOWN summary with the available-of-total count, configured-versus-effective chain per seat, mid-run unavailability transitions, and concise later-seat run-scoped skip lines), verbose composition diagnostics, prompt privacy in normal mode, and heartbeat stop behavior. The execution suite also covers the run-level OpenAI API execution override (`--mind openai-api`): configured Doctrine/Calling/Seat/cognitive/`maxResponseChars`/Memory identity unchanged, configured Preferred Mind preserved and never falsified, run override and actual execution target recorded structurally, the exact composed prompt reaching the API ask path unchanged, the configured browser fallback chain not walked, only the API Mind preflighted, over-target API output preserved, and structured API failures resolving through the Seat failure policy versus circuit-breaking the API Mind. A fifth deterministic suite (`npm run test:openai-transport`, `runner/src/openaiTransportAssertions.ts`) covers the shared OpenAI transport in isolation — success and each structured failure category mapped into Council ask semantics through an injected caller, `output_text` extraction, and key-presence reading — with no browser, runner service, network, or SDK call. These suites are separate commands so contract, execution-engine, cognitive-stat, operator-output, and transport regressions stay separately attributable, and none requires a browser, runner service, or network access. Separately, `npm run test:openai-api` is an isolated LIVE smoke test that makes one real billed Responses API call with exact-response validation; it is deliberately excluded from every deterministic/offline suite and must not be reported as passed unless actually run.
